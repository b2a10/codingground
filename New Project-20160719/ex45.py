        ...
        while curr_morale != success:
            ...
            if action == 1:
                ...
                if ini.response == 0:
                    ...
                    curr_morale = morale.increase()
                    print "Morale increased to %d" % curr_morale
                    cash = self.hood.decr_cash(ini)

                raw_input("> ")
            elif action == 3:
                print "Chose to build\nCurrent funds: %d" % cash

                bld = Build()
                cost = bld.create()

                if cost > cash:
                    print "Insufficient funds. Need $%d more." % (cost-cash)

                else:
                    print "Great idea! Morale increased."
                    cash -= cost
                    curr_morale = morale.increase() + 1

                raw_input("> ")
            else:
                print "You can't handle the stresses of being a Community Manager? Pathetic."
                break

            print "End of month %d. Press enter to continue." % self.hood.current_month
            raw_input("> ")
            self.hood.next_month()

            if self.hood.current_month == 12:
                self.hood.current_month = 0
                self.hood.increase_year()

        print "SUCCESS: Not yet configured."
...
class Fundraiser(Project):
    ...
    def create(self):
        ...
        if self.fund == 4 or self.fund == 7 or self.fund == 8:
            print "Not enough money raised to support this endeavor. Perhaps "
            print "try again when morale has been raised."
            self.response = 0
...
class Neighborhood(object):
    ...
    def decr_cash(self, event):
        ...
        elif isinstance(event, Fundraiser):
            if event.response == 0:
                self.cash -= 20
                print "Lost $%d from failed fundraiser" % 20
                return self.cash
